...
| Hunyuan3D | `HUNYUAN_SECRET_ID` + `HUNYUAN_SECRET_KEY` or `HUNYUAN_LOCAL_URL` | `ai3d.tencentcloudapi.com` v2025-05-13 or local `api_server.py` | TC3-HMAC-SHA256 signing, `SubmitHunyuanTo3DProJob` / `QueryHunyuanTo3DProJob`; local: `POST /send`, `GET /status/{uid}` returns `model_base64` |
