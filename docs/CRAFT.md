...
## Providers added
- Hunyuan 3D Engine (Tencent ai3d, Model "3.1", multi view up to 8, GenerateType Normal|LowPoly|Geometry|Sketch, PBR) plus post jobs SubmitTextureTo3DJob, SubmitHunyuanTo3DUVJob, SubmitAutoRiggingJob, SubmitReduceFaceJob; Hunyuan 3D World as guarded actions (`HUNYUAN_WORLD_SUBMIT` / `HUNYUAN_WORLD_QUERY` env, defaults SubmitHunyuanWorldJob / QueryHunyuanWorldJob, flagged unverified when Tencent answers InvalidAction); tools `maya_gen3d_hunyuan_post(job_id, op texture|uv|rig|reduce_face)` and `maya_gen3d_world(provider, prompt|image)`
- Replicate route (`REPLICATE_API_TOKEN`, provider name `replicate`): tencent/hunyuan-3d-3.1 by default, any hosted model via `options.extra.model = "owner/name"`; `POST /v1/models/{owner}/{name}/predictions` with `Prefer: wait=0`, `GET /v1/predictions/{id}`, output URL, list or dict

## Console
A gear button in the header opens a Settings dialog with a category list: General, Connection, AI 3D APIs (one row per provider: enable toggle, key fields with show/hide, Test button that pings the provider status endpoint from inside Maya with urllib), Asset Libraries, Live Link, Safe Mode. The old Settings tab is removed; keys still live in prefs.json (0600). `settings_dialog.py` keeps `PROVIDER_SPECS` and `test_provider()` free of Qt imports so they are unit tested without PySide2.
